# References with abstracts forQWIMproject: Network analysis and
...
ing.” In: _Advances in Neural Information Processing Systems 32 (NIPS 2019)_.
We consider the widely-used average-linkage, single-linkage, and Ward’s methods for computing hierarchical clus-
...
notoriously difficult problem. However, how fast can these algorithms be implemented if we allow approximation?
...
one could obtain a significant running-time improvement if the algorithm can merge -approximate closest clusters
(namely, clusters that are at distance (average, minimum, or sum-of-square error) at most times the distance of
the closest clusters). We show that one can indeed take advantage of the relaxation and compute the approximate
...
dilemma.” In: _Pattern Recognition_ 120, p. 108152.
...
is associated with a real-valued weight. To complement extensive formal analysis, we discuss how these properties
...
ability methods.” In: _Pattern Recognition_ 88, pp. 13–26.
...
Century Financial Crises.” In: _SSRN e-Print_.


We construct a network-based turbulence score that proves useful for analyzing the relationship between financial
...
and adversary-free training framework to learn robust models. Interestingly, _ClusTR_ outperforms adversarially-
...
_The Journal of Alternative Investments_ 22 (1), pp. 57–74.
...
hedge fund strategy has undesired network proximity (similarity) with strategies from other classifications and/or
...
only moderate or no connection to conventional portfolio optimization inputs and (ii) active investment strategies
...
Caldarelli, G. (2021).“The Physics of Financial Networks.” In: _arXiv e-Print_.
The field of Financial Networks is a paramount example of the novel applications of Statistical Physics that have
made possible by the present data revolution. As the total value of the global financial market has vastly outgrown
...
Barunik, J. and Ellington, M. (2021).“Dynamic Networks in Large Financial and Economic Systems.” In: _arXiv
e-Print_.
...
time series with an application to the US equity market.” In: _arXiv e-Print_.
...
_Journal of Machine Learning Research_ 21(185), pp. 1–6.
...
Bouveyron, C., Celeux, G., Murphy, T. B., and Raftery, A. E. (2019). _Model-Based Clustering and Classification
for Data Science: With Applications in R_. Cambridge University Press. 446 pp.
Model-Based Clustering and Classification for Data Science written by Bouveyron et al. provides a comprehensive
...
approach is model misspecification. To alleviate it, the authors employ mixture models After a historical overview
of clustering and classification given in Chapter 1, Chapter 2 introduces a family of Gaussian mixture models
(GMM), that is, (1) with Gaussian fg, for clustering. The GMM serves as a building block for the rest of chapters.
Under the GMM, the likelihood principle provides natural means for statistical inference. The computation of
the maximum likelihood estimates becomes handy thanks to the well-known expectation and maximization (EM)
...
with known membership denoted by z. This is the reason why this book talks about classification after clustering.
...
the observations and the variables simultaneously are also briefly mentioned. In conclusion, this book covers a
...
Brécheteau, C., Fischer, A., and Levrard, C. (2021).“Robust Bregman clustering.” In: _Annals of Statistics_ 49(3).
...
n denotes the sample size, under mild tail assumptions. We also show that it is robust to a certain amount of noise,
...
collect more and more data from daily operations, they expect to extract useful knowledge from existing collected
data to help make reasonable decisions for new customer requests, e.g. user credit category, confidence of expected
...
and diversification.
Campello, R. J. G. B., Kroger, P., Sander, J., and Zimek, A. (2020).“Density-based clustering.” In: _WIREs Data
Mining and Knowledge Discovery_ 10(2).
...
low-density regions are typically considered noise or outliers. In this review article we discuss the statistical notion
...
clustering.” In: _Advances in Data Analysis and Classification_ 15(3), pp. 599–623.
With the recent growth in data availability and complexity, and the associated outburst of elaborate modelling
approaches, model selection tools have become a lifeline, providing objective criteria to deal with this increasingly
...
case, where clusters are typically associated to the components of the selected mixture model, we define partitions
...
Relevant Number of Clusters in a Data Set.” In: _Journal of Statistical Software_.
...
random forests.” In: _Communications in Statistics - Simulation and Computation_ 50(2), pp. 426–445.
...
Chehreghani, M. H. (2021).“Shift of Pairwise Similarities for Data Clustering.” In: _arXiv e-Print_.
...
regularization of the pairwise similarities. This leads to shifting (adaptively) the pairwise similarities which might
...
different datasets.
Chen, W., Jiang, M., and Jiang, C. (2020).“Constructing a multilayer network for stock market.” In: _Soft computing_
24, pp. 6345–6361.
...
and Vogelstein, J. T. (2022).“Statistical Connectomics.” In: _Annual Review of Statistics and Its Application_
8(1), pp. 463–492.
...
and applications that are theoretically and empirically justified for analysis of connectome data. We hope this
...
approach based on the quadratic discriminant score.” In: _arXiv e-Print_.
...
Multivariate Panel Data.” In: _SSRN e-Print_.
We introduce a new method for dynamic clustering of panel data with dynamics for cluster location and shape,
cluster composition, and for the number of clusters. Whereas current techniques typically result in (economically)
...
Dalmia, A. and Sia, S. (2021).“Clustering with UMAP: Why and How Connectivity Matters.” In: _arXiv e-Print_.


Topology based dimensionality reduction methods such as t-SNE and UMAP have seen increasing success and
...
on UMAP, we study the effects of node connectivity (k-Nearest Neighbors vs _mutual_ k-Nearest Neighbors) and
...
reducing to 2 and 64 dimensions. Our findings indicate that a more refined notion of connectivity ( _mutual_
...
dependence structure.” In: _Journal of Banking & Finance_ 91, pp. 119–132.
...
on a multivariate copula approach.” In: _International Journal of Approximate Reasoning_ 139, pp. 88–103.
...
of approaches.” In: _arXiv e-Print_.
...
to data science applications.” In: _WIREs Computational Statistics_.
...
measures.” In: _Quantitative Finance_ 21(4), pp. 523–537.
...
Djouzi, K. and Beghdad-Bey, K. (2019).“A Review of Clustering Algorithms for Big Data.” In: _International
Conference on Networking and Advanced Systems (ICNAS)_. IEEE.
...
Doreian, P., Batagelj, V., and Ferligoj, A. (2020). _Advances in Network Clustering and Blockmodeling_. Wiley. 432 pp.
...
Duan, L. L. and Dunson, D. B. (2021).“Bayesian Distance Clustering.” In: _Journal of Machine Learning Research_
22(224), pp. 1–27.
...
density. Leveraging on properties of pairwise differences between data points, we propose a class of Bayesian
...
Clustering.” In: _IEEE Access_ 8, pp. 110775–110788.
...
approach.” In: _Neural Computing and Applications_ 33(19), pp. 12939–12956.
...
from low- to high-dimensional range as well as they are more robust to noise. To show the efficiency of our indexes
...
tering is obtained by the suggested data and is a formation of a recursive clustering. Several hierarchical clustering
...
networks.” In: _Proceedings of the Royal Society A: Mathematical, Physical and Engineering Sciences_ 476(2241),
p. 20190783.
...
propose a generalization of core-periphery structure to directed networks. Our approach yields a family of core-
...
Neural Network technology and support vector machine are identified as popular ML algorithms. A second review
...
In: _Journal of Business & Economic Statistics_ 39(2), pp. 453–465.
...
Esmaeili, S. A., Brubach, B., Tsepenekas, L., and Dickerson, J. P. (2021).“Probabilistic Fair Clustering.” In: _arXiv
e-Print_.
...
tres. Our contribution is substantially more efficient than k-means as it does not require an all to all comparison
...
Ezhilmaran, D. and Indira, D. V. (2020).“A survey on clustering techniques in pattern recognition.” In: _AIP
Conference Proceedings_. AIP Publishing.
...
Faletto, G. and Bien, J. (2022).“Cluster Stability Selection.” In: _arXiv e-Print_.
...
methods. With small modifications to the network architecture and training procedure, we show that dense neural
...
Ferraro, M. B., Giordani, P., and Serafini, A. (2019).“fclust: An R Package for Fuzzy Clustering.” In: _The R Journal_.
...
and outliers using exploratory projection pursuit.” In: _Communications in Statistics - Simulation and Compu-
tation_.
...
Fop, M. and Murphy, T. B. (2017).“Variable Selection Methods for Model-based Clustering.” In: _arXiv e-Print_.
...
In: _Pattern Recognition_ 93, pp. 95–112.
...
times reduces it further down to 1%. This accuracy is more than enough for most pattern recognition applications.
...
Fu, W. and Perry, P. O. (2020).“Estimating the Number of Clusters Using Cross-Validation.” In: _Journal of
Computational and Graphical Statistics_ 29(1), pp. 162–173.
...
Gagolewski, M. (2021).“genieclust: Fast and robust hierarchical clustering.” In: _SoftwareX_ 15, p. 100722.
...
speed, supports various distances over dense, sparse, and string data domains, and can be robustified even further
with the built-in noise point detector. As domain-independent software, it can be used for solving problems arising
...
Large-Scale Time Series Data.” In: _arXiv e-Print_.
...
the diverging dimension of the data in each computing unit and the sample sizeT. We use both simulated data
...
tering.” In: _The Journal of Financial Data Science_ 1 (4), pp. 84–102.
...
ment .” In: _Data & Knowledge Engineering_ 131-132, p. 101876.
...
evaluating clustering methods, artificial data is fundamental for assessing the performance since it allows creating
...
new findings that are not pointed out when using 1D and 2D artificial data The source code of the implementation
...
Techniques and Their Applications.” In: _Advances in Intelligent Systems and Computing_. Springer Singapore,
pp. 69–83.
In modern world, we have to deal with huge volumes of data which include image, video, text and web documents,
...
Portfolio Management.” In: _Frontiers in Artificial Intelligence_ 3.
...
Robot advisory services are rapidly expanding, responding to a growing interest people have in directly managing
...
by robot-advisors.
Goel, A. and Majumdar, A. (2021).“Transformed K-means Clustering.” In: _arXiv e-Print_.
...
such a naïve piecemeal fashion. The K-means clustering loss is embedded into the transform learning framework
...
Grun, B. (2018).“Model-based Clustering.” In: _arXiv e-Print_.
...
on different ways to specify the cluster model is given. Post-processing methods to determine a suitable clustering,
...
Guan, S. and Loew, M. (2021).“A Distance-based Separability Measure for Internal Cluster Validation.” In: _arXiv
e-Print_.
...
tering.” In: _AIP Conference Proceedings_. AIP Publishing.
This study presents a robust portfolio selection with time series clustering. The stocks are firstly grouped into
several clusters using Partitioning Around Medoids (PAM) time series clustering base on autocorrelation function
...
Clustering Based on the Characterization of Segment Typologies.” In: _IEEE Transactions on Cybernetics_.
...
method. However, these approaches do not take the similarity of the different subsequences of each time series into
...
Guo, L., Hardle, W. K., and Tao, Y. (2021).“A Time-Varying Network for Cryptocurrencies.” In: _arXiv e-Print_.
...
Gupta, K. and Chatterjee, N. (2018).“Financial Time Series Clustering.” In: _Information and Communication
Technology for Intelligent Systems (ICTIS 2017) - Volume 2_. Ed. by S. C. Satapathy and A. Joshi. Vol. 84.
Smart Innovation, Systems and Technologies. Springer International Publishing, pp. 146–156.


Financial time series clustering finds application in forecasting, noise reduction and enhanced index tracking.
...
market situations.” In: _Expert systems with applications_ 147 (113226).
Dimensionality reduction is inevitable in stock selection with cluster analysis. Considering relations among di-
mensionality reduction, noise trading, and market situations, we empirically investigate the effect of dimensionality-
reduction methods-principal component analysis, stacked autoencoder, and stacked restricted Boltzmann machine-
...
_Physica A: Statistical Mechanics and its Applications_ 604, p. 127704.
...
Herteliu, C., Levantesi, S., and Rotundo, G. (2021).“Network analysis of pension funds investments.” In: _Physica
A: Statistical Mechanics and its Applications_ 579, p. 126139.
...
experiments with real datasets show. This method – dubbed the Wassersteink-means algorithm – frames such
a problem as one on the space of probability measures with finitepthmoment, in terms of thep-Wasserstein
...
markets based on multilayer networks.” In: _Physica A: Statistical Mechanics and its Applications_ 586, p. 126445.
...
shock such as the outbreak of COVID-19 epidemic and the meltdown of stock markets. Furthermore, sensitivity
...
“Learning From Networks: Algorithms, Theory, and Applications.” In: _Proceedings of the 25th ACM SIGKDD
International Conference on Knowledge Discovery & Data Mining_. ACM.
...
quick recollection of the exciting history of the area, we formulate the core technical problems. Then, we introduce
...
A Survey.” In: _International Journal of Computer Applications_ 134(7), pp. 9–14.
...
This paper covers the survey of various clustering techniques, the current similarity measures based on distance
...
Jackson, M. O. and Pernoud, A. (2020).“Systemic Risk in Financial Networks: A Survey.” In: _SSRN e-Print_.
...
The Hierarchical risk parity (HRP) approach of portfolio allocation, introduced by Lopez de Prado (2016), applies
...
Javed, A., Lee, B. S., and Rizzo, D. M. (2020).“A Benchmark Study on Time Series Clustering.” In: _arXiv e-Print_.
...
In: _Finance Research Letters_ 46 (Part B) (102424).
...
using differential evolution.” In: _Proceedings of the Genetic and Evolutionary Computation Conference_. ACM.
...
synthetic datasets are considered: linearly separable data having both well-separated and overlapped clusters, and
non-linearly separable data having arbitrarily-shaped clusters. Besides, real-life datasets are also considered. The
experimental results indicate that the Silhouette index consistently reached an acceptable performance in linearly
...
market networks.” In: _arXiv e-Print_.


Maximum spanning tree (MST) is a popular tool in market network analysis. Large number of publications are
devoted to the MST calculation and it’s interpretation for particular stock markets. However, much less attention
...
Katsouris, C. (2021).“Optimal Portfolio Choice and Stock Centrality for Tail Risk Events.” In: _arXiv e-Print_.
...
munity detection.” In: _arXiv e-Print_.
...
Financial Time Series.” In: _International Conference on Software, Telecommunications and Computer Networks
(SoftCOM)_. IEEE.
...
Multi-Factor Strategies.” In: _Risk & Reward_ , pp. 26–32.
...
A Practical Network Approach.” In: _The Journal of Financial Data Science_ 2 (1), pp. 24–44.
...
The results show that equity and bond funds have a significant exposure to the Fama-French-Carhart factors. The
...
_The Journal of Financial Data Science_ 22(3) (3), pp. 55–72.


In this article, the authors investigate the hedge fund market as a network of interacting individual funds.
...
Kumar, S., Bansal, A., and Chakrabarti, A. S. (2021).“Ripples on financial networks.” In: _The European Journal
of Finance_ , Early View.
...
of centroid.” In: _International Journal of Advanced Technology and Engineering Exploration (IJATEE)_ 7(63),
pp. 48–52.
...
with stability-based relative clustering validation.” In: _arXiv e-Print_.
...
that replicates, via supervised learning, on unseen subsets of data. The package works with multiple clustering
...
Modelling Diagrams of Dendrogram Trees.” In: _Carpathian Journal of Electronic and Computer Engineering_
13(3), pp. 5–12.
...
include Matplotlib, SciPy, NumPy, Pandas by Python and dendextend, pvclust, magrittr by R. The dendrograms
...
Hierarchical clustering algorithms achieve the best financial performance obtaining an adequate trade-off between
accumulated financial returns and the risk-adjusted measure, Omega Ratio, during the out of sample testing
period.
Leopold, N. and Rose, O. (2020).“UNIC: A fast nonparametric clustering.” In: _Pattern Recognition_ 100, p. 107117.
...
control parameters. We present a new fast nonparametric clustering algorithm, UNIC, to address these challenges.
...
Li, H. and Liu, Z. (2021).“Multivariate time series clustering based on complex network.” In: _Pattern Recognition_
115, p. 107919.
...
Li, T., Levina, E., and Zhu, J. (2020).“Network cross-validation by edge sampling.” In: _Biometrika_ 107(2), pp. 257–
276.
While many statistical models and methods are now available for network analysis, resampling of network data
remains a challenging problem. Cross-validation is a useful general tool for model selection and parameter tuning,
...
Lipor, J. and Balzano, L. (2020).“Clustering quality metrics for subspace clustering.” In: _Pattern Recognition_ 104,
p. 107328.
...
In: _The Journal of Financial Data Science_ (2), 2, pp. 51–63.
...
_The Journal of Financial Data Science_ 1(1), pp. 99–110.
...
Lopez de Prado, M. (2019b).“Estimation of Theory-Implied Correlation Matrices.” In: _SSRN e-Print_.
Correlation matrices are ubiquitous in finance. Some key applications include portfolio construction, risk manage-
...
Lopez de Prado, M. (2020a).“Clustering.” In: _SSRN e-Print_.
...
Expectation-Maximization framework for Unsupervised Clustering driven by Supervised Learning.” In: _arXiv
e-Print_.
...
generalization capability of our algorithm using synthetic and experimental datasets. In particular, we validate its
...
Python package athttps://github.com/neurospin-projects/2021_rlouiset_ucsl.


Lu, Y., Li, M., Tang, X., and Wang, H. (2021).“A Cluster Representative Selection Method for Stock Portfolio
Based on Efficient Frontier.” In: _IEEE 24th International Conference on Computer Supported Cooperative Work
in Design (CSCWD)_. IEEE.
Portfolio is a financial concept to combine several stocks to reduce the risks and improve the profits. To choose the
basic members of portfolio, we can group similar stocks into one cluster and then choose representative stock from
...
Ma, E. (2020). _Network Analysis Made Simple_ .url:https://ericmjl.github.io/Network-Analysis-Made-
Simple/.
...
Time Series Clustering and Classification includes relevant developments on observation-based, feature-based and
...
and students.
Mahfuz, N. M., Yusoff, M., and Ahmad, Z. (2019).“Review of single clustering methods.” In: _IAES International
Journal of Artificial Intelligence (IJ-AI)_ 8(3), p. 221.
...
and Heavy Tailed Time Series.” In: _Symmetry_ 13(6), p. 959.
...
framework for clustering possibly skewed and heavy tailed time series. This paper develops a clustering procedure
...
for network reconstruction and graph distances.” In: _arXiv e-Print_.
...
performance varies from system to system in the real world. One way around this problem could be to use
...
_Artificial Intelligence Review_ 53(8), pp. 5995–6023.
...
proximity measure. Second, clustering algorithms of different categories have been overviewed and experimentally
...
Rank Correlation.” In: _arXiv e-Print_.
...
Millington, T. and Niranjan, M. (2020b).“Partial correlation financial networks.” In: _Applied Network Science_ 5(1)
(11).
...
times of turbulence?” In: _Physica A: Statistical Mechanics and its Applications_ 574, p. 126016.
...
and how this varies over time, using daily returns from the S&P500 (US), FTSE100 (UK) and DAX30 (Germany).


We study both the full correlation networks and those filtered using the PMFG method. We conclude that stocks
tend to become more similar in the full correlation networks during times of market disruption for the US and UK
...
fication compatible with scikit-learn.” In: _arXiv e-Print_.
...
and documentation are available athttps://gitlab.com/dacs-hpi/hiclass.
...
Montero, P. and Vilar, J. A. (2015).“TSclust: An R Package for Time Series Clustering.” In: _Journal of Statistical
Software_ 62.
...
supported in TSdist, directly enabling the evaluation and comparison of their performance within these two
frameworks.
Murialdo, P., Ponta, L., and Carbone, A. (2021).“Inferring multi-period optimal portfolios via detrending moving
average cluster entropy.” In: _EPL (Europhysics Letters)_ 133(6), p. 60004.
...
does indeed improve the out-of-sample return and Sharpe ratio. However, the resulting portfolio is also associated
...
changes in various assets’ relationships in financial markets.” In: _arXiv e-Print_.
...
methods to sovereign bond yields during COVID-19.” In: _Physica A: Statistical Mechanics and its Applications_
574, p. 125995.
...
and Explainable Machine Learning for Constructing Robust Investment Portfolios.” In: _The Journal of Financial
...
order similarities.” In: _Pattern Recognition_ 121, p. 108108.
...
states in the space of correlation matrices with applications to financial markets.” In: _arXiv e-Print_.
...
for clustering algorithms.” In: _Knowledge-Based Systems_ 195, p. 105682.


One of the main challenges in Clustering Analysis is choosing the optimal number of clusters. A typical method-
...
Markowitz Portfolio Optimization.” In: _Modeling and Analysis of Information Systems_ 27(1), pp. 62–71.
...
R package for Comparison and Validation of communities.” In: _arXiv e-Print_.
...
framework.” In: _Computers & Operations Research_ 117, p. 104891.
...
network with a metric based on correlation coefficients between assets’ returns, and show how classical location
...
B-Spline Based Clustering.” In: _Procedia Computer Science_ 179, pp. 375–382.
...
Rahgoshay, M. and Salavatipour, M. R. (2021).“Hierarchical Clustering: New Bounds and Objective.” In: _arXiv
e-Print_.
Hierarchical Clustering has been studied and used extensively as a method for analysis of data. More recently,
Dasgupta [2016] defined a precise objective function. Given a set ofndata points with a weight functionwi,j
for each two itemsiandjdenoting their similarity/dis-similarity, the goal is to build a recursive (tree like)
partitioning of the data points (items) into successively smaller clusters. He defined a cost function for a treeT
to beCost(T) =
...
```
whereTi,jis the subtree rooted at the least common ancestor ofiandjand
```
```
...
and average linkage have approximation ratio1/3which has been improved in a series of works to 0. 585 [Alon
et al. 2020]. Later Cohen-Addad et al. [2019] considered the same objective function as Dasgupta’s but for
dissimilarity-based metrics, calledRev(T). It is shown that both random partitioning and average linkage have
ratio2/3which has been only slightly improved to 0. 667078 [Charikar et al. SODA2020]. Our first main result
is to considerRev(T)and present a more delicate algorithm and careful analysis that achieves approximation
0. 71604. We also introduce a new objective function for dissimilarity-based clustering. For any treeT, letHi,j
be the number ofiandj’s common ancestors. Intuitively, items that are similar are expected to remain within
the same cluster as deep as possible. So, for dissimilarity-based metrics, we suggest the cost of each treeT,
which we want to minimize, to beCostH(T) =
```
#### ∑
...
Rehman, A. U. and Belhaouari, S. B. (2022).“Divide well to merge better: A novel clustering algorithm.” In: _Pattern
Recognition_ 122, p. 108305.
...
Romashchenko, A. (2021).“Clustering with Respect to the Information Distance.” In: _arXiv e-Print_.
...
Rusch, T., Mair, P., and Hornik, K. (2021).“Cluster Optimized Proximity Scaling.” In: _Journal of Computational
and Graphical Statistics_ , pp. 1–12.
...
through Hierarchical Clustering.” In: _arXiv e-Print_.
...
as individual observations across large collections of time series data. Our visualization provides a unique way
...
financial indices.” In: _arXiv e-Print_.
...
of correlations between them. Here, we adopt a similar network-based approach to analyse the daily closing prices
...
are either related to the distance measure used to assess dissimilarity, or the function used to calculate prototypes
...
portfolio, which puts equal weight on each asset, is apart from its simplicity a strategy which is hard to outperform
...
disadvantages are investigated. Using clustering techniques the possible assets are separated into non-overlapping
...
Sato-Ilic, M. (2021).“Cluster-scaled principal component analysis.” In: _WIREs Computational Statistics_.
...
In: _EPJ Data Science_ 10(1).
To monitor risk in temporal financial networks, we need to understand how individual behaviours affect the
global evolution of networks. Here we define a structural importance metric – which we denote asle– for the
edges of a network. The metric is based on perturbing the adjacency matrix and observing the resultant change in
...
capability of predicting whether an edge will change from its value ofle. We then estimate the model parameters
...
Algorithm.” In: _Bioinformation_ 13(03), pp. 101–103.
...
outbreak. The involved countries are US, China, Taiwan, Korea, Japan and some others. The clustering results by
...
Networks Approach.” In: _arXiv e-Print_.
...
and algorithms.” In: _WIREs Data Mining and Knowledge Discovery_ 11(3).
...
their advantages and limitations in a unified way and language. We first describe the case of univariate time series,
...
“VARCLUST: clustering variables using dimensionality reduction.” In: _arXiv e-Print_.
...
on Graphs Part I: Graphs, Graph Spectra, and Spectral Clustering.” In: _Foundations and Trends in Machine
Learning_ 13 (1).
...
properties, such as cuts, walks, paths, and neighborhoods. Next, to illustrate estimation strategies performed
...
Analytics on Graphs Part II: Signals on Graphs.” In: _Foundations and Trends in Machine Learning_ 13 (2-3).
...
subsampling, are presented and further linked with compressive sensing. The principles of time-varying signals on
...
Analytics on Graphs Part III: Machine Learning on Graphs, from Graph Topology to Applications.” In: _Foun-
dations and Trends in Machine Learning_.
Many modern data analytics applications on graphs operate on domains where graph topology is not known a
priori, and hence its determination becomes part of the problem definition, rather than serving as prior knowledge
...
vertices reside on a high-dimensional regular lattice structure. This part of monograph concludes with two
...
phenomena such as bubbles or crashes. Motivated by these uncertainties, we designed a novel causality framework
...
embedded in reconstructed state space. Using a broad class of real-world applications, we are able to demonstrate
...
Tang, W., Xu, X., and Zhou, X. Y. (2021).“Asset Selection via Correlation Blockmodel Clustering.” In: _arXiv
e-Print_.
...
Algorithm with Automatic Estimation of the Number of Clusters.” In: _PLOS ONE_ 11(3), e0152333+.
Four of the most common limitations of the many available clustering methods are: i) the lack of a proper strategy
...
enough to be successfully used in such diverse applications. The algorithm has been implemented in the statistical
...
Thrun, M. C. (2021).“The Exploitation of Distance Distributions for Clustering.” In: _International Journal of
Computational Intelligence and Applications_ 20(03).
...
study. Depending on the data context, different properties for distance distributions are judged to be relevant
...
Thrun, M. C. and Stier, Q. (2021).“Fundamental clustering algorithms suite.” In: _SoftwareX_ 13, p. 100642.
...
the number of clusters.” In: _Neurocomputing_ 458, pp. 655–666.
...
Turner, E. (2021).“Graph Auto-Encoders for Financial Clustering.” In: _arXiv e-Print_.
...
the culmination of multiple streams allows for cross source pattern recognition that would have otherwise gone
...
networks.” In: _Applied Network Science_ 5(1) (62).
...
poorly on directed networks. One approach is to capture and cluster higher-order structures using motif adjacency
...
Valk, M. and Cybis, G. B. (2021).“U-Statistical Inference for Hierarchical Clustering.” In: _Journal of Computational
and Graphical Statistics_ 30(1), pp. 133–143.
Clustering methods are valuable tools for the identification of patterns in high-dimensional data with applications
...
Data Representations.” In: _arXiv e-Print_.
Dimensionality reduction and clustering techniques are frequently used to analyze complex data sets, but their
results are often not easy to interpret. We consider how to support users in interpreting apparent cluster structure
...
time series clustering.” In: _Neurocomputing_ 457, pp. 182–192.
...
knowledge representation is propose to balance the aggregation of the distances. The final clustering is performed
...
Vojtko, R. and Cisár, D. (2021).“An Analysis of Volatility Clustering of Equity Factor Strategies.” In: _SSRN e-Print_.
...
nar maximally filtered graph, and a threshold significance graph) and use three centrality measures (betweenness,
...
transaction costs.
Wadhwa, R. R. and Scott, J. G. (2020).“Exploring complex networks with the ICON R package.” In: _arXiv e-Print_.
...
reproducible report can be found athttps://github.com/rrrlw/ICON.
Wang, M., Abrams, Z. B., Kornblau, S. M., and Coombes, K. R. (2018).“Thresher: determining the number of
clusters while removing outliers.” In: _BMC Bioinformatics_ 19(1), p. 9.
...
Algorithm Selection.” In: _arXiv e-Print_.
...
Wang, Y. and Tsay, R. S. (2019).“Clustering Multiple Time Series with Structural Breaks.” In: _Journal of Time
Series Analysis_ 40(2), pp. 182–202.
...
detection of the number of groups. Using Monte Carlo simulation, we document the performance of the proposed
...
Clustering via Algorithmic Regularization.” In: _arXiv e-Print_.
...
similarities in nearest neighbor graphs.” In: _Knowledge-Based Systems_ 228, p. 107295.


Hierarchical clustering allows better performance in grouping heterogeneous and non-spherical datasets than
...
network reconstruction.” In: _Knowledge-Based Systems_ 193, p. 105406.
...
_Physica A: Statistical Mechanics and its Applications_ 531, p. 121813.
...
risks for financial networks.” In: _arXiv e-Print_.
In this study, we propose a new multi-objective portfolio optimization with idiosyncratic and systemic risks for
financial networks. The two risks are measured by the idiosyncratic variance and the network clustering coefficient
...
Yelibi, L. and Gebbie, T. (2021).“Agglomerative Likelihood Clustering.” In: _arXiv e-Print_.
...
estimates of stability for clusters, observations and model selection.” In: _Computational statistics_ 34(1), pp. 349–
372.
...
approach to estimating the stability of a clustering method, which also captures stability of the individual clusters
...
visualization extrapolated from the stability profiles that give insights into the separation of groups, and projected
...
covariance matrix and allow the creation of an optimal and robust portfolio. We compared the resulting portfolios
...
Zhang, M. (2021).“Weighted Clustering Ensemble: A Review.” In: _arXiv e-Print_.
...
Community detection methods can be used to explore the structure of complex systems. The well-known modular
...
In: _Physica A: Statistical Mechanics and its Applications_ 506, pp. 1104–1112.
...
global graph information.” In: _Neurocomputing_ 449, pp. 15–23.
...
from the co-association matrix.” In: _Pattern Recognition_ 92, pp. 93–106.
...
In: _Knowledge-Based Systems_ 193, p. 105512.
...
seriously affect the clustering performance since the original data usually contain noises, even redundant features.
...
clustering.” In: _Knowledge-Based Systems_ 193, p. 105417.
...
data, such balanced structure is not obvious in the original feature space, due to the noisy and redundant
